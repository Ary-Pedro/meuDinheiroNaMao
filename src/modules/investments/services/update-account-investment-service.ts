...
import type { UpdateAccountInvestmentDto } from "../dto/account-investment-dto";
...
export class UpdateAccountInvestmentService {
  constructor(
    private readonly accountsRepository: AccountsRepository,
    private readonly accountInvestmentsRepository: AccountInvestmentsRepository
  ) {}

  async execute(userId: string, investmentId: string, input: UpdateAccountInvestmentDto): Promise<AccountInvestmentResponse> {
    ...
    const investment = await this.accountInvestmentsRepository.update({
      userId,
      investmentId,
      ...input,
    });
