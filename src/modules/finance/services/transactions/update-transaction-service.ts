...
export class UpdateTransactionService {
  ...
  async execute(input: UpdateTransactionDto & { userId: string; transactionId: string }): Promise<TransactionResponse> {
    const existing = await this.transactionsRepository.findById(input.userId, input.transactionId);
    if (!existing) {
