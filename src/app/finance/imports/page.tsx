...
import {
  ...
  getImportedEntryStatusLabel,
  getReviewItemStatusLabel,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Área secundária do financeiro para validar lotes importados antes de virar dado real."
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard label="Entradas em análise" value={String(overview.totals.reviewRequiredEntries)} />
        <StatCard label="Lotes em análise" value={String(overview.totals.reviewRequiredBatches)} />
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <SectionCard title="Fila de conciliação">
          {overview.queue.length ? (
            ...
            <EmptyState title="Nenhum item pendente" description="Quando houver divergências de importação, elas aparecerão aqui." />
          )}
        </SectionCard>
              ...
              {overview.batches.map((batch) => (
                <div key={batch.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    {batch.totalEntries} entrada(s) • {batch.reviewRequiredEntries} em análise
                  </p>
                </div>
              ))}
            </div>
          ) : (
            <EmptyState title="Sem lotes" description="Os lotes de importação aparecerão aqui." />
          )}
        </SectionCard>
      </div>
    </div>
